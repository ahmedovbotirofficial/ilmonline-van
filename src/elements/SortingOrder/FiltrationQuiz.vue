      ...
      <div class="sorting_select__wrapper">
        <div class="sorting_select__sort">
          <div class="sorting_select__list">
            <div
              v-for="(item, idx) in filtersList"
              :key="idx"
              class="level__select"
              @click="selectFiltersBy(item)"
            >
              <div
                class="level__checkbox"
                :class="{ selected: filterBy.includes(item.slug) }"
              >
                <div
                  v-if="filterBy.includes(item.slug)"
...
export default {
  ...
  methods: {
    ...
    selectFiltersBy(item) {
      // this.leaveHover();
      this.$emit('selectFiltersBy', item.slug);
    },
  },
};
</script>
